...
def create_featurize_fn(idx, mdp, mlam):
    fns = [
        create_standard_fn,
        create_featurize_1,
        create_featurize_2,
...
def create_featurize_1(self, mlam):
    # Just own data and relative distances
    def featurize_state(states, joint_actions, num_pots=2, **kwargs):
        """
        ...
        overcooked_state = states[-1]
        features_np = get_feat_dict(overcooked_state, self, mlam, num_pots)
...
def create_featurize_2(self, mlam):
    # Just own data and relative distances and what the other is holding
    def featurize_state_2(states, joint_actions, num_pots=2, **kwargs):
        """
        Encode state with some manually designed features. Works for arbitrary number of players
        ...
        for i, player_i in enumerate(overcooked_state.players):
            ...
            player_i_abs_pos = player_absolute_positions[i]
            player_i_rel_pos = player_relative_positions[i]
            player_i_other_holding = []
            for j, player_j in enumerate(overcooked_state.players):
                if player_i == player_j:
                    continue
                pj_held = features_np[f"p{j}_objs"]
                player_i_other_holding.append(pj_held)
            player_i_other_holding = np.concatenate(player_i_other_holding)
...
def create_featurize_3(self, mlam):
    # Just own data plus relative distance plus what the rest is holding plus everybody's last 5 actions (player-centric)
    def featurize_state_3(states, joint_actions, num_pots=2, **kwargs):
        """
        Encode state with some manually designed features. Works for arbitrary number of players
        ...
        for i, player_i in enumerate(overcooked_state.players):
            ...
            last_actions = []
            for j in range(5):
                if len(joint_actions) > j:
                    current_action = joint_actions[-(j+1)][i]
                    last_actions.append(np.eye(len(Action.ALL_ACTIONS))[Action.ACTION_TO_INDEX[current_action]])
                else:
                    last_actions.append(np.zeros(len(Action.ALL_ACTIONS)))
            last_actions = np.concatenate(last_actions)

            player_features.append(features)
            player_absolute_positions.append(abs_pos)
            player_relative_positions.append(rel_pos)
            player_last_actions.append(last_actions)
        ...
        for i, player_i in enumerate(overcooked_state.players):
            ...
            for j, player_j in enumerate(overcooked_state.players):
                ...
                player_i_other_holding.append(pj_held)
            player_i_other_holding = np.concatenate(player_i_other_holding)
            player_i_last_actions = player_last_actions[i]
            player_i_other_last_actions = []
            for j, player_j in enumerate(overcooked_state.players):
                if player_i == player_j:
                    continue
                pj_last_actions = player_last_actions[j]
                player_i_other_last_actions.append(pj_last_actions)
            player_i_other_last_actions = np.concatenate(player_i_other_last_actions)
    ...
    return featurize_state_3


def create_featurize_4(self, mlam):
    # Both player's data plus previous 5 actions (player-centric)
    def featurize_state_4(states, joint_actions, num_pots=2, **kwargs):
        """
        Encode state with some manually designed features. Works for arbitrary number of players
    ...
    return featurize_state_4


def create_featurize_5(self, mlam):
    # This and previous entire state plus previous action
    def featurize_state_5(states, joint_actions, num_pots=2, **kwargs):
        """
        Encode state with some manually designed features. Works for arbitrary number of players
        ...
        overcooked_state = states[-1]
        features_1 = self.featurize_state(overcooked_state, mlam)
        if len(states) > 2:
            features_2 = self.featurize_state(states[-2], mlam)
        else:
            features_2 = [np.zeros(96) for _ in overcooked_state.players]

        player_last_actions = []
        for i, player_i in enumerate(overcooked_state.players):
            if len(joint_actions) > 0:
                current_action = joint_actions[-1][i]
                last_action = np.eye(len(Action.ALL_ACTIONS))[Action.ACTION_TO_INDEX[current_action]]
            else:
                last_action = np.zeros(len(Action.ALL_ACTIONS))
            player_last_actions.append(last_action)

        ordered_features = []
        for i, player_i in enumerate(overcooked_state.players):
            player_i_last_actions = player_last_actions[i]
            other_player_last_actions = []
            for j, player_j in enumerate(overcooked_state.players):
                if player_j == player_i:
                    continue
                other_player_last_actions.append(player_last_actions[j])
            other_player_last_actions = np.concatenate(other_player_last_actions)
            features = np.concatenate([features_1[i], features_2[i], player_i_last_actions, other_player_last_actions])
            ordered_features.append(features)

        return ordered_features

    return featurize_state_5


def get_feat_dict(overcooked_state, self, mlam, num_pots):
    all_features = {}
